...
// Selectividad paramétrica
 ...
 init_bounded_number log_rangof(-4,0.6,opt1_fase)
  ...
  { 
    ...
    Nvp  = Nv(nanos);
      ...
      { 
        ...
        //------------------------------------------------------------------
        if(oprec==1) // considera el reclutamiento promedio historico
        {
        Np(1)  = exp(log_Ro+0.5*square(sigmaR));
        Nvp(1) = exp(log_Ro+0.5*square(sigmaR));
        }
              if(oprec==2) // reclutamiento promedio ultimos 8 años
              {
              Np(1)  = mean(Reclutas(nanos-9,nanos)); 
              Nvp(1) = mean(Reclutas(nanos-9,nanos));
              }
                  if(oprec==3)  // reclutamiento promedio hasta 2008
                   {
                     Np(1)  = mean(Reclutas(1,nanos-8)); 
                     Nvp(1) = mean(Reclutas(1,nanos-8));   
                   }
       //------------------------------------------------------------------
